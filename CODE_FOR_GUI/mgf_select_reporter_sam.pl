...
# IN ANOTHER FOLDER.
# THE THIRD PARAMETER IS THE MZ_ERROR
# THE FOURTH PARAMETER IS THE TYPE (eg:TMT10)
# THE FIFTH IS MIN INTENSITY
# THE SIXTH IS MIN REPORTERS
...
if ($ARGV[0]=~/\w/) { $write_dir=$ARGV[1];} else { exit 1}
if ($ARGV[1]=~/\w/) { $mz_error=$ARGV[2];} else { exit 1 } 
if ($ARGV[2]=~/\w/) { $type=$ARGV[3];} else { exit 1}
if ($ARGV[3]=~/\w/) { $min_intensity=$ARGV[4];} else { exit 1}
if ($ARGV[4]=~/\w/) { $min_reporters=$ARGV[5];} else { exit 1}
